...
if 20 <= args.ostype <= 29:
    data['builders'][0]["boot_wait"] = "1s"
    data['builders'][0]["boot_command"] = ["""<enter><wait40>bash -c 'pacman -Sy --noconfirm git wget && {gitcmd} && /opt/CustomScripts/ZSlimDrive.py -n && /opt/CustomScripts/BManjaro.py -n -c "{vmname}" -u "{vmuser}" -f "{fullname}" -q "{vmpass}" /mnt && reboot'<enter>""".format(vmname=vmname, vmuser=args.vmuser, vmpass=args.vmpass, fullname=args.fullname, gitcmd=git_cmdline())]
    data['builders'][0]["ssh_handshake_attempts"] = "9999"
    data['provisioners'][0]["type"] = "shell"
