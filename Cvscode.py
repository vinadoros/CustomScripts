...
def cmd_pips(cmd_type=int, enabled=bool):
    ...
    if enabled is True and cmd_type == 2:
        subprocess.run("flatpak run --command=pip3 com.visualstudio.code install {0} --user".format(pip_packages), shell=True, check=True)
    # Windows
...
# Flatpak
code_array[2]["cmd"] = ["flatpak", "run", "--command=code", "com.visualstudio.code"]
if shutil.which("flatpak") and cmd_silent(code_array[2]["cmd"] + ["-h"]) == 0:
    code_array[2]["en"] = True
else:
    code_array[2]["en"] = False
code_array[2]["path"] = os.path.join(userhome, ".var", "app", "com.visualstudio.code", "config", "Code", "User")
