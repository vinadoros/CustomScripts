...
def getrandomport():
    ...
    while True:
        ...
        randomport = random.randint(50000, 60000)
...
if __name__ == '__main__':
    ...
    if args.command == 0:
        ...
        """.format(networkport))
    if args.command == 1:
        subprocess.run('{0} localhost:{1}'.format("vncviewer", networkport), shell=True, check=False)
    if args.command == 2:
        subprocess.run('ssh localhost -p {0}'.format(networkport), shell=True, check=False)
