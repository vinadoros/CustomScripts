...
if type kwriteconfig5; then
	...
	kwriteconfig5 --file "$HOME/.local/share/konsole/Profile 1.profile" --group "Scrolling" --key HistoryMode 2
	kwriteconfig5 --file "$HOME/.local/share/konsole/Profile 1.profile" --group "Appearance" --key ColorScheme Solarized
	kwriteconfig5 --file "$HOME/.local/share/konsole/Profile 1.profile" --group "Appearance" --key Font "Liberation Mono,11,-1,5,50,0,0,0,0,0,Regular"
...
if type xfconf-query &> /dev/null && type xfce4-panel &> /dev/null; then
	...
	if [[ $VBOXGUEST = 1 || $QEMUGUEST = 1 || $VMWGUEST = 1 ]]; then
		...
		xfconf-query -c xfce4-power-manager -p /xfce4-power-manager/dpms-enabled --type bool --set false --create
